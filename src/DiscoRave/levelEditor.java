...
public class levelEditor extends JPanel {
	...
	static String[] notifs = {"Warning: This level is currently unplayable.",
							"Make sure the start and end points are on empty panels."};
				...
				while (true) { 
					...
					**/
					...
					for(int n=0;n<notifs.length;n++)
						if(msgs.all.get(n).dir==2){
							msgs.all.get(n).dir = 0;
							msgs.all.get(n).active = false;
	...
	public static void toFile(){
		try {
			if(level.index!=-1)
				source = new File("").getCanonicalPath() + "/Levels/Official/level" + level.index + ".txt";
			else
				source = new File("").getCanonicalPath() + "/Levels/level" + level.name + ".txt";
        ...
        try {
            ...
            for(int c = 0;c<cnt;c++)
            	//if(!(Integer.parseInt(trans[c].split(",")[4])==4||Integer.parseInt(trans[c].split(",")[4])==7))
            		printWriter.println(trans[c]);
            for(String s:saveFile)
            	printWriter.println(s);
            printWriter.println(level.author+",0,0,0,-4");
	...
	public static void main(String[] args) throws IOException{
		...
		if(level.index!=-1)
			source = level.filePath + "Official/level"+level.index+".txt";
		else
			source = level.filePath + "level"+level.name+".txt";
		...
		for(int b=0;b<buttons.length;b++){
			String btnName = "";
			buttons[b] = new Button((boxWidth/2)-300,200 + 125*b,600,100,btnName);
			switch(b){
				case 0:
					btnName = "Delete Map";
						...
						if(buttons[b].hover==true)
							if(b==0){
								...
								if(level.index!=-1){
									...
									n = JOptionPane.showOptionDialog(frame,
											"Are you sure you want to delete Level " + level.name + "?" , "Warning",
											JOptionPane.YES_NO_OPTION,
											JOptionPane.QUESTION_MESSAGE,
											null,
											options,
									...
									try{
										...
										if(levelFile.delete()==true){
											if(level.index!=-1)
												...
												JOptionPane.showMessageDialog(frame, "Level " + level.name + " deleted!");
											...
											if(level.index!=-1)
												...
												JOptionPane.showMessageDialog(frame, "Delete operation failed. Level " + level.name + " does not exist.");
										}
									}catch(Exception f){f.printStackTrace();}
								}
							}else if(b==10){
								toFile();
								String lvl = JOptionPane.showInputDialog(frame, "Enter level number/name:");
								try{
								...
								if(gameEngine.isLoaded==false){
									...
									gameEngine.level.name = level.name;
			...
			public void mouseReleased(MouseEvent e) {
				if(level.pause==false){
					...
					if(type==4&&e.getButton()==MouseEvent.BUTTON1){
						level.panels[(int)(e.getX()-4+cam.pos.x)/size][(int)(e.getY()-25+cam.pos.y)/size].startPoint = true;
						level.panels[(int)level.oldStart.x][(int)level.oldStart.y].startPoint = false;
						if(level.panels[(int)(e.getX()-4+cam.pos.x)/size][(int)(e.getY()-25+cam.pos.y)/size].exist==true){
							msgs.activateMessage(notifs[0]);
							msgs.activateMessage(notifs[1]);
						}
						saveFile.add((int)level.oldStart.x+","+(int)level.oldStart.y+",0,0,-2");	
						level.oldStart.x = (e.getX()-4+cam.pos.x)/size;
						level.oldStart.y = (e.getY()-25+cam.pos.y)/size;
						...
						level.panels[(int)(e.getX()-4+cam.pos.x)/size][(int)(e.getY()-25+cam.pos.y)/size].endPoint = true;
						level.panels[(int)level.oldEnd.x][(int)level.oldEnd.y].endPoint = false;
						saveFile.add((int)level.oldEnd.x+","+(int)level.oldEnd.y+",0,0,-3");
						if(level.panels[(int)(e.getX()-4+cam.pos.x)/size][(int)(e.getY()-25+cam.pos.y)/size].exist==true){
							msgs.activateMessage(notifs[0]);
							msgs.activateMessage(notifs[1]);
						}
						level.oldEnd.x = (e.getX()-4+cam.pos.x)/size;
						level.oldEnd.y = (e.getY()-25+cam.pos.y)/size;
						saveFile.add((int)(e.getX()-4+cam.pos.x)/size+","+(int)(e.getY()-25+cam.pos.y)/size+",0,0,7");	
	...
	public void paintComponent(Graphics g) {
		...
		for(int x=(int)Math.max(0,(cam.pos.x/size)+0);x<(int)Math.min(lvl.size.x,(cam.pos.x+boxWidth+50)/size);x++){
			for(int y=(int)Math.max(0,(cam.pos.y/size)+0);y<(int)Math.min(lvl.size.y,(cam.pos.y+boxHeight+50)/size);y++){
				...
				if(lvl.panels[x][y].check==true){
					g.setColor(Color.cyan);
					g.fillOval((int)(size*x-cam.pos.x-30),(int)(size*y-cam.pos.y-30), 60,60);
				}
				if(lvl.panels[x][y].startPoint==true){
					g.setColor(Color.ORANGE);
					g.fillOval((int)(size*x-cam.pos.x-30),(int)(size*y-cam.pos.y-30), 60,60);
				}
				if(lvl.panels[x][y].endPoint==true){
		...
		if(start.equals(new Vector2(-1,-1))==false&&type<4){
			...
			g.fillRect(size*((int)sx/size), size*((int)sy/size), (size*((int)ex/size)) - (size*((int)sx/size)) , (size*((int)ey/size)) - (size*((int)sy/size)) );
		...
		if(type<4){
			...
			g.fillRect((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size, size);
			
			g.setColor(Color.GRAY);
			g.drawRect((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size, size);
		}else{
			switch(type){
			case 4:
				g.setColor(Color.ORANGE);
				g.fillArc((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size * 3, size * 3,90,180);
				g.setColor(Color.MAGENTA);
				g.fillArc((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size * 3, size * 3,270,180);
				break;
			case 5:
				g.setColor(Color.CYAN);
				g.fillArc((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size * 3, size * 3,90,180);
			...
			g.setColor(Color.GRAY);
			g.drawOval((int)level.player.marker.x + 5, (int)level.player.marker.y + 5, size * 3, size * 3);
		}
	...
	public static void resetEditor(){
		level.resetLevel(false);
		cam.pos.reset();
		frame.setCursor(levelEditor.myCursor);
		saved = "";
	}
	/**
